...
!!.................................................................
  ...
  alpha_iter = 0.08_R8
   ...
   ! eps_reg = 0.0016_R8 
   ! alpha_iter = 0.00025_R8
   ! eps_reg = 0.00015_R8
   ! alpha_iter = 0.000024_R8
...
!   flux_con_max_iter = 2000 ! use for ctrl run with adaptive limiter
   ...
   DO n=1,nMax
     if (mask(n) /= 0) then
      ...
      !       ! iterate until convergence or max iters are reached
      !       iter = 0
      !       do while( sqrt(((ustar - ustar_prev)/ustar)**2 + ((tstar - tstar_prev)/tstar)**2 + ((qstar - qstar_prev)/qstar)**2 + ((u10n - u10n_prev)/u10n)**2) > flux_con_tol .and. &
      !                 iter < flux_con_max_iter)
      !          iter = iter + 1
      !          ustar_prev = ustar
      !          tstar_prev = tstar
      ...
      !    !---- end of method ---
         ...
         !--- new method with convergence and uniqueness fix and regularization! ---
         !--- set zeta limiter and incrementer ---
         ! we first apply the limiter with zeta_max = 20 and
         ! check whether the iteration converges to zeta = zeta_max.
         ! if so, reduce the value of zeta_max by zeta_incr and try again.
        ...
        do while ((abs(abs(hol) - zeta_max) < 1.e-6) .and. (zeta_max > 0.0))
            ...
            ! iterate until convergence or max iters are reached
            iter = 0
            ! abstols: ustar: 1e-3, u10n: 1e-3, tstar: 1e-6, qstar: 1e-9
            do while( sqrt(((ustar_next - ustar_prev)/(ustar_prev + 1.e-3_R8))**2 + ((tstar_next - tstar_prev)/(tstar_prev + 1.e-5_R8))**2 + ((qstar_next - qstar_prev)/(qstar_prev + 1.e-8_R8))**2 + ((u10n_next - u10n_prev)/(u10n_prev + 1.e-3_R8))**2) > flux_con_tol )
            ! do while( sqrt(((ustar - ustar_prev)/ustar)**2 + ((tstar - tstar_prev)/tstar)**2 + ((qstar - qstar_prev)/qstar)**2 + ((u10n - u10n_prev)/u10n)**2) > flux_con_tol )
               ! write(s_logunit,*) flux_con_tol,flux_con_max_iter,"testing new alg"
               ! write(s_logunit,*) "iter=", iter, ", res=", sqrt(((ustar - ustar_prev)/(ustar + 0.e-2_R8))**2 + ((tstar - tstar_prev)/(tstar + 0.e-4_R8))**2 + ((qstar - qstar_prev)/(qstar + 0.e-6_R8))**2 + ((u10n - u10n_prev)/(u10n + 0.e-2_R8))**2), ", zeta=", hol
               ! abort if exceeded max iters (use only for final runs)
               if (iter > flux_con_max_iter) then
                  write(s_logunit,*) vmag,thbot(n),ts(n),zbot(n),qbot(n),ssq,ustar,ustar_prev,tstar,tstar_prev,qstar,qstar_prev,flux_con_tol,flux_con_max_iter
                  call shr_sys_abort('MAX ITERS (1000) REACHED WITHOUT OCN-ATM ITERATION CONVERGING ' // errMsg(sourcefile, __LINE__))
               end if
            ...
            ! write(s_logunit,*) "Iters for convergence ", iter, ", zeta_max=", zeta_max, ustar, tstar, qstar, ", res=", sqrt(((ustar_next - ustar_prev)/(ustar_prev + 0.e-2_R8))**2 + ((tstar_next - tstar_prev)/(tstar_prev + 0.e-4_R8))**2 + ((qstar_next - qstar_prev)/(qstar_prev + 0.e-6_R8))**2 + ((u10n_next - u10n_prev)/(u10n_prev + 0.e-2_R8))**2), ", zeta=", hol
