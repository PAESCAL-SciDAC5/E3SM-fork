...
main() {
    ...
    #subroutine name
    sub_name=advance_clubb_core_api

    #ins and in-outs variables
    #(a comma seprated string, like 'var1, var2, var3'- remember the quotes around the list of variables)
    ins='wpthlp_sfc,wprtp_sfc,upwp_sfc,rho_ds_zm,thv_ds_zm,thlm,rtm'

    #outs and in-outs variables
    #(a comma seprated string, same as above)
    outs='thlm, rtm, rcm_in_layer, cloud_cover'

    #module name
    module_name=clubb_api_module

    #file path
    dir_path=components/eam/src/physics/clubb/

    #CPP directive to turn on file writing
    cpp_directive=YAML_ADVCLUBB
    ...
    if [ $this_dir != "maint-3.0" ]; then
        echo 'Please run this script at source code root in directory "maint-3.0"'
        exit 1
    fi
...
create_code_for_in_outs () {
    ...
    python3 $MY_PATH/gen_input_write_vars.py  --finp $3 --fout $4 -i "$1" -o "$2"
