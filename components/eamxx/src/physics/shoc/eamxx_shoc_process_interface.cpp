...
size_t SHOCMacrophysics::requested_buffer_size_in_bytes() const
{
  ...
  const size_t wsm_request= WSM::get_total_bytes_needed(nlevi_packs, 15+(n_wind_slots+n_trac_slots), policy);
...
void SHOCMacrophysics::init_buffers(const ATMBufferManager &buffer_manager)
{
...
#ifdef SCREAM_SHOC_SMALL_KERNELS
    , &m_buffer.rho_zt, &m_buffer.shoc_qv, &m_buffer.tabs, &m_buffer.dz_zt, &m_buffer.shoc_thv
#endif
  };
  ...
  const int n_wind_slots = ekat::npack<Spack>(2)*Spack::n;
  const int n_trac_slots = ekat::npack<Spack>(m_num_tracers+3)*Spack::n;
  const int wsm_size     = WSM::get_total_bytes_needed(nlevi_packs, 15+(n_wind_slots+n_trac_slots), policy)/sizeof(Spack);
  s_mem += wsm_size;
...
void SHOCMacrophysics::initialize_impl (const RunType run_type)
{
  ...
  workspace_mgr.setup(m_buffer.wsm_data, nlevi_packs, 15+(n_wind_slots+n_trac_slots), default_policy);
