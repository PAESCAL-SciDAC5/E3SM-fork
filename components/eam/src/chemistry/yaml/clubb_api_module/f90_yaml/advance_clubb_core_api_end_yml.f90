  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'thlm',thlm(:))
        call write_var(unit_output,'rtm',rtm(:))
        call write_var(unit_output,'rcm_in_layer',rcm_in_layer(:))
        call write_var(unit_output,'cloud_cover',cloud_cover(:))
